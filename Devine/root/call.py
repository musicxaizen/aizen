...
class Call(PyTgCalls):
    ...
    async def stream_call(self, link):
        assistant = await group_assistant(self, configuration.LOGGER_ID)
        await assistant.join_group_call(
            configuration.LOGGER_ID,
            AudioVideoPiped(link),
            stream_type=StreamType().pulse_stream,
        )
        await asyncio.sleep(0.2)
        await assistant.leave_group_call(configuration.LOGGER_ID)
    ...
    async def change_stream(self, client, chat_id):
        ...
        try:
            ...
            if "live_" in queued:
                ...
                run = await app.send_photo(
                    chat_id=original_chat_id,
                    photo=configuration.STREAM_IMG_URL,
                ...
                if videoid == "telegram":
                    ...
                    run = await app.send_photo(
                        chat_id=original_chat_id,
                        photo=configuration.TELEGRAM_AUDIO_URL
                        if str(streamtype) == "audio"
                        else configuration.TELEGRAM_VIDEO_URL,
                        caption=_["stream_1"].format(
                            configuration.SUPPORT_CHAT, title[:23], check[0]["dur"], user
                        ),
                        reply_markup=InlineKeyboardMarkup(button),
                    )
                    db[chat_id][0]["mystic"] = run
                    ...
                    run = await app.send_photo(
                        chat_id=original_chat_id,
                        photo=configuration.SOUNCLOUD_IMG_URL,
                        caption=_["stream_1"].format(
                            configuration.SUPPORT_CHAT, title[:23], check[0]["dur"], user
                        ),
                        reply_markup=InlineKeyboardMarkup(button),
