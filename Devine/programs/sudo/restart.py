...
async def update_(client, message, _):
    ...
    to_exc = f"git fetch origin {configuration.UPSTREAM_BRANCH} &> /dev/null"
    os.system(to_exc)
    await asyncio.sleep(7)
    verification = ""
    REPO_ = repo.remotes.origin.url.split(".git")[0]
    for checks in repo.iter_commits(f"HEAD..origin/{configuration.UPSTREAM_BRANCH}"):
    ...
    for info in repo.iter_commits(f"HEAD..origin/{configuration.UPSTREAM_BRANCH}"):
