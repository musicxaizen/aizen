...
def get_image(videoid):
    if os.path.isfile(f"cache/{videoid}.png"):
        ...
        return configuration.YOUTUBE_IMG_URL
...
async def get_queue(client, message: Message, _):
    ...
    if "live_" in file:
        ...
        IMAGE = configuration.STREAM_IMG_URL
    else:
        if videoid == "telegram":
            IMAGE = (
                configuration.TELEGRAM_AUDIO_URL
                if typo == "Audio"
                else configuration.TELEGRAM_VIDEO_URL
            )
        elif videoid == "soundcloud":
            IMAGE = configuration.SOUNCLOUD_IMG_URL
        else:
            IMAGE = get_image(videoid)
...
@app.on_callback_query(filters.regex("GetTimer") & ~filter_users)
...
@app.on_callback_query(filters.regex("GetQueued") & ~filter_users)
@languageCB
...
@app.on_callback_query(filters.regex("queue_back_timer") & ~filter_users)
@languageCB
