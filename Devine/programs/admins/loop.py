...
prefixes = ["/", "!", "."]
command_filters = filters.command(["loop", "cloop"]) & filters.group & ~filter_users

@app.on_message(filters.group & ~filter_users & (filters.command(["loop", "cloop"]) | filters.regex(f"^[{'|'.join(prefixes)}]loop$|^[{'|'.join(prefixes)}]cloop$")))
@AdminRightsCheck
